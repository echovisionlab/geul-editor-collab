...
export function createDependencyManifest(packageJson) {
  ...
  const lifecycleScripts = Object.fromEntries(
    INSTALL_LIFECYCLE_SCRIPTS.filter((name) => packageJson.scripts?.[name]).map(
...
async function main() {
  ...
  if (!sourcePath || !dependencyPath || !buildPath) {
    throw new Error(
      "usage: docker-package-manifests.mjs <source> <dependency-output> <build-output>",
    );
  }
  ...
  await Promise.all([
    writeFile(
      dependencyPath,
      `${JSON.stringify(createDependencyManifest(packageJson), null, 2)}\n`,
    ),
    writeFile(
      buildPath,
      `${JSON.stringify(createBuildManifest(packageJson), null, 2)}\n`,
    ),
  ]);
}
